...
Composition-only Aspire helpers for hosts that run Concertable Search migrations, Web, and Workers. The package exposes
Search resource names and Azure Service Bus topology without referencing the Search runtime implementation.

Run the migration resource to successful completion before starting either runtime resource. Project-backed
overloads support Search's standalone developer host; image-backed overloads support system composition.
